...
abstract class GetMemberInCourseRemoteDataSource {
  Future<GetMemberInCourseResponse> getMemberInCourse(
      String idCourse, String keySearchName);
}

class GetMemberInCourseRemoteDataSourceImpl
    implements GetMemberInCourseRemoteDataSource {
  ...
  Future<GetMemberInCourseResponse> _getMemberInCourse(
    ...
    final response = await client!.get(
      Uri.parse('$mainUrl/course/GetMemberInCourse/$idCourse$keySearchName'),
      headers: {
    ...
    if (response.statusCode == 200) {
      var success =
          GetMemberInCourseResponse.fromJson(json.decode(response.body));
