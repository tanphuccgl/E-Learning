...
class GetPointStudentBloc
    extends Bloc<GetPointStudentEvent, GetPointStudentState> {
  ...
  @override
  Stream<GetPointStudentState> mapEventToState(
      GetPointStudentEvent event) async* {
    if (event is GetPointStudentEventE) {
      yield Loading();
      final failureOrSuccess = await getPointStudent(GetPointStudentParams(
          idCourse: event.idCourse, idAccount: event.idAccount));
